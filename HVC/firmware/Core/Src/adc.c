...
uint16_t hvc_adc_read_voltage_sense_raw(void)
{
   ...
   */
  return adc_read_raw(&hadc1, ADC_CHANNEL_6, ADC_SAMPLETIME_2CYCLES_5, ADC_DIFFERENTIAL_ENDED);
}
...
uint16_t hvc_adc_read_current_sense_raw(void)
{
   ...
   * Returns raw differential ADC value.
   */
  return adc_read_raw(&hadc2, ADC_CHANNEL_3, ADC_SAMPLETIME_2CYCLES_5, ADC_DIFFERENTIAL_ENDED);
}
...
float hvc_adc_read_current_sense_a(void)
{
   ...
   * Reference voltage: 3300 mV (3.3V)
   * Assuming a current sense amplifier with gain (adjust based on actual design):
   * If using a 1mV/A sensor with 10x gain amplifier: 10mV/A output
   * Voltage per LSB: 3.3V / 4096 = 0.000805 V/LSB
   * Current per LSB: 0.000805 / (10mV/A) = 0.0805 A/LSB (for 10mV/A sensitivity)
   * 
   * Adjust the scaling factor (10.0f) based on your actual amplifier gain and sensor output.
   */
  ...
  float current_a = voltage_v / 0.01f;  // Adjust 0.01 based on your sensor sensitivity
  return current_a;
}
/* USER CODE END 1 */
