...
const osThreadAttr_t defaultTask_attributes = {
    .name = "defaultTask", .priority = osPriorityNormal, .stack_size = 128 * 4};
...
void StartDefaultTask(void* argument) {
    // USB MUST init inside a running RTOS task don't mess with this Alice
    // you've done fucked up once
    MX_USB_Device_Init();
...
void StartTorqueTask(void* argument) {
    ...
    for (;;) {
        ...
        log_printf(
            LOG_INFO,
            "APP1=%u (%d.%03d)  APP2=%u (%d.%03d)  ped_f=%d.%03d  tq=%d.%02d "
            "Nm  impl=%d  brake=%d\r\n",
            raw1, p1_i / 1000, p1_i % 1000, raw2, p2_i / 1000, p2_i % 1000,
            pf_i / 1000, pf_i % 1000, tq_i / 100, tq_i % 100,
            apps_implaus ? 1 : 0, brake_latched ? 1 : 0);
