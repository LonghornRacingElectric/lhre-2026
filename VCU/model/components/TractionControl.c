...
void traction_control_evaluate(const vcu_inputs_t *in, vcu_outputs_t *out,
  ...
  float alpha_raw = (omega - omega_prev) / 0.003f;
  ...
  float error = alpha_raw - params->traction_control.alpha_threshold_rad_s2;
  if (error > 0.0f) {
  ...
  if(trim > out->torque_power_limited) {
    trim = out->torque_power_limited;
  }
