...
#define CMAKE_INTDIR "."
/** \def CMAKE_INTDIR                                                        \
    \brief The macro #CMAKE_INTDIR gives intermediate directory for the      \
    current configuration (Debug/Release).                                   \
                                                                           \ \
It is redefined to "." for Makefile based build environments. Otherwise is   \
set by MSVC. */                                                              \
#endif
...
/** \brief Terminals and Icons XML file name */
...
\brief The macro #FMILIB_STATIC_LIB_ONLY is defined if no fmilib_shared is \
build. */                                                                  \
#endif
...
#if 0 /* defined(__GNUC__)  MSYS currently does not support visibility \
attribute */
...
/** \def FMILIB_EXPORT
    \brief Used to declare the public API of the library needed for dynamic
    linking.
    \def FMILIB_PRIVATE
...
\brief Activates debug level log messages. If not defined the debug messages
are compiled out.
*/
...
/** \brief Get the library build stamp.
    ...
    The function is only active if the library was configured with
    #FMILIB_GENERATE_BUILD_STAMP ON
*/
