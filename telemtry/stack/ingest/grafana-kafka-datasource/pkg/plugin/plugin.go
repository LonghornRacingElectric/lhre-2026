...
// FilterCondition represents a server-side predicate filter for Kafka messages.
type FilterCondition struct {
	Field    string `json:"field"`
	Operator string `json:"operator"` // eq, neq, gt, lt, gte, lte, contains, not_contains, regex
...
type queryModel struct {
	...
	// Server-side throttling: minimum milliseconds between accepted messages per partition
	SampleIntervalMs int32 `json:"sampleIntervalMs"`
}
...
func (d *KafkaDatasource) CallResource(ctx context.Context, req *backend.CallResourceRequest, sender backend.CallResourceResponseSender) error {
	...
	if req.Path == "discover-fields" && req.Method == "GET" {
		return d.handleDiscoverFields(ctx, req, sender)
	}
...
// handleDiscoverFields samples a few recent messages from the topic, flattens them,
// and returns the sorted list of unique field names. Used by the frontend to populate
// the "Fields" dropdown.
func (d *KafkaDatasource) handleDiscoverFields(ctx context.Context, req *backend.CallResourceRequest, sender backend.CallResourceResponseSender) error {
	parsedURL, err := url.Parse(req.URL)
	if err != nil {
		return sendJSON(sender, 400, map[string]string{"error": "Invalid URL"})
	}
	...
	messageFormat := q.Get("messageFormat")
	if messageFormat == "" {
		messageFormat = "json"
	}
	// How many messages to sample (default 5, max 50)
	sampleLimit := 5
	if s := q.Get("limit"); s != "" {
	...
	partitions, err := d.client.GetTopicPartitions(ctx, topic)
	if err != nil {
		if errors.Is(err, kafka_client.ErrTopicNotFound) {
			return sendJSON(sender, 404, map[string]string{"error": fmt.Sprintf("topic %s not found", topic)})
		}
		return sendJSON(sender, 500, map[string]string{"error": fmt.Sprintf("Failed to get partitions: %s", err.Error())})
	}
	if len(partitions) == 0 {
		return sendJSON(sender, 200, map[string]interface{}{"fields": []string{}})
	}

	// Sample from the first partition, starting from the most recent N messages.
	// Use a short context deadline so the UI doesn't hang if the topic is idle.
	sampleCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()

	reader, err := d.client.NewStreamReader(sampleCtx, topic, partitions[0], "lastN", int32(sampleLimit))
	if err != nil {
		return sendJSON(sender, 500, map[string]string{"error": fmt.Sprintf("Failed to create reader: %s", err.Error())})
	}
	defer reader.Close()

	// Minimal StreamConfig so decoders know the format (Avro/Protobuf decoding requires
	// schemas which aren't available here — only JSON discovery is meaningful for now).
	cfg := &StreamConfig{MessageFormat: messageFormat}
	...
	for i := 0; i < sampleLimit; i++ {
		...
		msg, err := d.client.ConsumerPull(sampleCtx, reader, messageFormat)
		if err != nil {
			// Timeout or no more messages — return what we have
			break
		}

		// Decode JSON if needed
		var value interface{} = msg.Value
		if messageFormat == "json" && value == nil && len(msg.RawValue) > 0 {
			dec := json.NewDecoder(strings.NewReader(string(msg.RawValue)))
			dec.UseNumber()
		...
		// Wrap top-level array the same way ProcessMessageToFrame does
		if arr, ok := value.([]interface{}); ok {
			wrapped := make(map[string]interface{})
			for i, el := range arr {
				wrapped[fmt.Sprintf("item_%d", i)] = el
			}
			value = wrapped
		}

		flat := make(map[string]interface{})
		FlattenJSON("", value, flat, 0, d.settings.FlattenMaxDepth, d.settings.FlattenFieldCap)
		for k := range flat {
			fieldSet[k] = struct{}{}
		}
	}
	...
	_ = cfg // reserved for future Avro/Protobuf discovery
	return sendJSON(sender, 200, map[string]interface{}{
		"fields": fields,
		"topic":  topic,
	})
...
func (d *KafkaDatasource) RunStream(ctx context.Context, req *backend.RunStreamRequest, sender *backend.StreamSender) error {
	...
	streamConfig := &StreamConfig{
		...
		RefID:                qm.RefID,
		Alias:                qm.Alias,
		SelectedFields:       qm.SelectedFields,
		FilterConditions:     qm.FilterConditions,
		SampleIntervalMs:     qm.SampleIntervalMs,
	}
	...
	messageCount := 0
	// Per-partition last-accepted timestamps for server-side sampling/throttling.
	lastAcceptedAt := make(map[int32]time.Time)
	sampleInterval := time.Duration(streamConfig.SampleIntervalMs) * time.Millisecond
	for {
		select {
		case <-ctx.Done():
		...
		case msgWithPartition := <-messagesCh:
			...
			// Server-side throttling: drop messages that arrive within sampleInterval of the
			// last accepted message for this partition.
			if sampleInterval > 0 {
				now := time.Now()
				if last, ok := lastAcceptedAt[msgWithPartition.partition]; ok && now.Sub(last) < sampleInterval {
					log.DefaultLogger.Debug("Dropping message due to sample interval",
				...
				lastAcceptedAt[msgWithPartition.partition] = now
			}
			...
			// nil frame with nil error means the message was filtered out by server-side predicates
			if frame == nil {
				log.DefaultLogger.Debug("Message filtered out by server-side predicates",
					"messageNumber", messageCount,
					"partition", msgWithPartition.partition,
					"offset", msgWithPartition.msg.Offset)
				continue
