...
type StreamConfig struct {
	...
	SampleIntervalMs int32
}

// matchesFilterConditions checks whether a flattened message matches all configured filter conditions.
// Returns true if no conditions are configured or all conditions pass.
func matchesFilterConditions(flat map[string]interface{}, conditions []FilterCondition) bool {
	if len(conditions) == 0 {
		return true
	}
	for _, cond := range conditions {
		...
		val, exists := flat[cond.Field]
		if !exists {
			// Field not present in message — treat as non-match for all operators except neq/not_contains
			if cond.Operator == "neq" || cond.Operator == "not_contains" {
				continue
			}
			return false
		}
...
// evaluateCondition checks a single value against an operator and comparison value.
func evaluateCondition(fieldVal interface{}, operator, condValue string) bool {
	...
	case "neq":
		return fieldStr != condValue
	case "contains":
		return strings.Contains(fieldStr, condValue)
	case "not_contains":
		return !strings.Contains(fieldStr, condValue)
	case "regex":
		matched, err := regexp.MatchString(condValue, fieldStr)
		return err == nil && matched
	case "gt", "lt", "gte", "lte":
		return compareNumeric(fieldVal, operator, condValue)
	default:
		return true
	}
}
...
func compareNumeric(fieldVal interface{}, operator, condValue string) bool {
...
// applyFieldSelection filters a flattened map to only include the specified fields.
// If selectedFields is empty, all fields are kept.
func applyFieldSelection(flat map[string]interface{}, selectedFields []string) map[string]interface{} {
	if len(selectedFields) == 0 {
		return flat
	}
	allowed := make(map[string]struct{}, len(selectedFields))
	for _, f := range selectedFields {
		allowed[f] = struct{}{}
	}
	filtered := make(map[string]interface{}, len(selectedFields))
	for k, v := range flat {
...
func ProcessMessageToFrame(client KafkaClientAPI, msg kafka_client.KafkaMessage, partition int32, partitions []int32, config *StreamConfig, topic string, flattenMaxDepth int, flattenFieldCap int, fieldBuilder *FieldBuilder) (*data.Frame, error) {
	...
	includeMeta := func(name string) bool {
		if len(config.SelectedFields) == 0 {
			return true
		}
		for _, f := range config.SelectedFields {
			if f == name {
				return true
			}
		}
	...
	if len(partitions) > 1 && includeMeta("partition") {
	...
	// Apply server-side predicate filtering — drop messages that don't match
	if !matchesFilterConditions(flat, config.FilterConditions) {
		return nil, nil // Signal to caller: skip this message
	}

	// Apply server-side field selection — only keep requested fields
	flat = applyFieldSelection(flat, config.SelectedFields)
