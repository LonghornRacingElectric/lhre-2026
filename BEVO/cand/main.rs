...
mod config;
use config::{PacketConfig, ProtobufMapping};
            ...
            "bitfield" => {
                if let Some(mappings) = &signal.bitfield_encoding {
                    for m in mappings {
                        ...
                        update_proto_bool(data, &m.protobuf_field, is_set);
                    }
                }
                continue;
            }
...
fn update_proto_field(data: &mut SensorData, name: &str, val: f32, config: &ProtobufMapping) {
    let d = data.dynamics.get_or_insert_with(Default::default);
    let c = data.controls.get_or_insert_with(Default::default);
    let p = data.pack.get_or_insert_with(Default::default);
    let l = data.diagnostics_low.get_or_insert_with(Default::default);

    if config.repeated {
        if let Some(i) = config.field_index {
            match name {
                "fl_sprung_accel" => set_vec_index(&mut d.fl_sprung_accel, i, val),
                "fr_sprung_accel" => set_vec_index(&mut d.fr_sprung_accel, i, val),
                _ => (),
            }
        }
    } else {
        match name {
            "fl_steer_angle" => d.fl_steer_angle = val,
            "fr_steer_angle" => d.fr_steer_angle = val,
            "flw_speed" => d.flw_speed = val,
            "frw_speed" => d.frw_speed = val,
            "brake_pressure_f" => c.brake_pressure_f = val,
            "hv_pack_v" => p.hv_pack_v = val,
            "hv_c" => p.hv_c = val,
            "hv_soc" => l.hv_soc = val,
            _ => (),
        }
...
fn update_proto_bool(data: &mut SensorData, name: &str, val: bool) {
    let h = data.diagnostics_high.get_or_insert_with(Default::default);
    let l = data.diagnostics_low.get_or_insert_with(Default::default);
    match name {
        "apps1_disconnect" => h.apps1_disconnect = val,
        "apps2_disconnect" => h.apps2_disconnect = val,
        "bmb_comm_error" => l.bmb_comm_error = val,
        "imd_gnd_isolation_error" => l.imd_gnd_isolation_error = val,
        _ => (),
    }
}

fn set_vec_index(v: &mut Vec<f32>, i: usize, val: f32) {
