...
echo "[1/8] Marking helper scripts executable"
chmod +x "$SCRIPT_DIR/launch_kiosk.sh"

echo "[2/8] Installing systemd unit for static dash server (repo=$REPO_ROOT)"
...
echo "[3/8] Installing bevo_telemetry systemd unit (installed, NOT enabled)"
...
echo "[4/8] Installing Chromium-kiosk autostart entry"
mkdir -p "$HOME/.config/autostart"
sed "s|__BEVO_REPO__|$REPO_ROOT|g" "$DESKTOP_SRC" > "$DESKTOP_DST"

echo "[5/8] Overriding labwc system autostart (no panel, no desktop)"
...
echo "[6/8] Installing Plymouth boot-splash theme (black)"
...
echo "[7/8] Verifying static server is up"
sleep 1
...
# take effect.
echo "[8/8] Disabling serial console (frees GPIO14 for Locus Lock power)"
