...
const uint32_t LOOP_MEASUREMENT_COUNT = 4;      /* Loop measurment count */
const uint16_t MEASUREMENT_LOOP_TIME  = 2;     /* milliseconds(mS)*/
...
void adBms6830_read_cell_voltages(uint8_t tIC, cell_asic *ic)
{
  ...
  adBms6830_Unsnap();
  // printVoltages(tIC, &ic[0], Cell);  // Disabled for embedded use
}
