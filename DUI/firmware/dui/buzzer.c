...
#define BUZZER_TEST_DURATION_MS 2000U
...
static void buzzer_task(void *argument) {
  ...
  bool r2d_was_enabled = is_r2d_enabled();
  ...
  while (true) {
    ...
    if (r2d_enabled && !r2d_was_enabled) {
      uint32_t cycles =
          (BUZZER_TEST_DURATION_MS * 1000U) / BUZZER_HALF_PERIOD_US;
