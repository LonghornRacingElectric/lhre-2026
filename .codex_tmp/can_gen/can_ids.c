...
int pack_motor_cooling(const msg_motor_cooling_t* msg, uint8_t* tx_buf) {
    memset(tx_buf, 0, MOTOR_COOLING_DLC);
    ...
    // Pack: Radiator Fan Speed
    uint16_t raw_radiator_fan_speed = (uint16_t)(msg->radiator_fan_speed / 0.2f);
    tx_buf[6 + 0] = (uint8_t)(raw_radiator_fan_speed & 0xFF);
    tx_buf[6 + 1] = (uint8_t)((raw_radiator_fan_speed >> 8) & 0xFF);

    return 0;
}

int unpack_motor_cooling(const uint8_t* rx_buf, msg_motor_cooling_t* msg) {
    ...
    // Unpack: Radiator Fan Speed
    uint16_t raw_radiator_fan_speed = 0;
    raw_radiator_fan_speed = (uint16_t)rx_buf[6 + 0];
    raw_radiator_fan_speed |= (uint16_t)(rx_buf[6 + 1] << 8);
    msg->radiator_fan_speed = (float)raw_radiator_fan_speed * 0.2f;
