...
void charging_init(int a) {
  ...
  sFilterConfigHVC.FilterMode =
      CAN_FILTERMODE_IDMASK; // Use Mask Mode for ID range
  sFilterConfigHVC.FilterScale =
      CAN_FILTERSCALE_32BIT; // 32-bit scale for Extended IDs

  // Set the Extended ID (29-bit) - Left-align it for the hardware register
  // Conficured for the ChargingLimits Can ID
  // sFilterConfigHVC.FilterIdHigh = (0x1806E5F4 >> 13) & 0xFFFF; // Upper 16
  // bits sFilterConfigHVC.FilterIdLow = (0x1806E5F4 << 3) & 0xFFF8;   // Lower
  // 13 bits + IDE bit
  ...
  sFilterConfigElcon.FilterMode =
      CAN_FILTERMODE_IDMASK; // Use Mask Mode for ID range
  sFilterConfigElcon.FilterScale =
      CAN_FILTERSCALE_32BIT; // 32-bit scale for Extended IDs

  // Set the Extended ID (29-bit) - Left-align it for the hardware register
  // Conficured for the Status Can ID
  sFilterConfigElcon.FilterIdHigh =
      (0x18FF50E5 >> 13) & 0xFFFF; // Upper 16 bits
  sFilterConfigElcon.FilterIdLow =
      (0x18FF50E5 << 3) & 0xFFF8; // Lower 13 bits + IDE bit
...
void angeliqueLoop(float dt) {
  ...
  while (HAL_CAN_GetRxFifoFillLevel(hvc, CAN_RX_FIFO1) > 0) {
    RxHeader.ExtId = 0;
    uint32_t error =
        HAL_CAN_GetRxMessage(hvc, CAN_RX_FIFO1, &RxHeader, commandData);
    if (error != HAL_OK) {
      Error_Handler();
    }
    ...
    // commandData[2]=0;
    // commandData[3]= (int) (curLimit*10);  // overriding to 10.0A for
    // ChargePoint, 2.5 for wall charger so we dont blow up pickle
    //
    // rainbow |= 0x01;
  }
  ...
  while (HAL_CAN_GetRxFifoFillLevel(elcon, CAN_RX_FIFO0) > 0) {
    RxHeader.ExtId = 0;
    uint32_t error =
        HAL_CAN_GetRxMessage(elcon, CAN_RX_FIFO0, &RxHeader, statusData);
    if (error != HAL_OK) {
      Error_Handler();
    }
  ...
  if (currTime > nextTime) {
    ...
    uint32_t mailbox2 = 0;
    uint32_t error3 =
        HAL_CAN_AddTxMessage(elcon, &TxToHVC, chargerPluggedPacket, &mailbox2);
    if (error3 != HAL_OK) {
      Error_Handler();
    }
    ...
    if (rainbow & 0x01) {
      ...
      uint32_t mailbox = 0;
      uint32_t error2 =
          HAL_CAN_AddTxMessage(elcon, &TxHeader, commandData, &mailbox);
      if (error2 != HAL_OK) {
        Error_Handler();
      }
    }
    ...
    if (rainbow & 0x02) {
      ...
      uint32_t mailbox = 0;
      uint32_t error2 =
          HAL_CAN_AddTxMessage(hvc, &TxHeader, statusData, &mailbox);
      if (error2 != HAL_OK) {
        Error_Handler();
      }
    }
...
bool getAmsError() { return amsError; }

bool getImdError() { return imdError; }
